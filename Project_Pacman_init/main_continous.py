...
def imageinit():
    ...
    global pacman_orient,pacman_previous_orient
...
def initsize():
    ...
    global pacman_previous_x,pacman_previous_y
    ...
    main_screen=pygame.display.set_mode((2*mapboarder_buffer+windows_length+320,\
        windows_height+2*mapboarder_buffer))
...
def input_file():
    ...
    map_input.close()
    pacman_x=pacman_y=0
    pacman_previous_x=pacman_previous_y=0
    pacman_previous_orient=pacman_orient=0
...
def randomized_start():
    ...
    global pacman_x,pacman_y,pacman_orient
    global blockgeneratePossibly
    global pacman_previous_x,pacman_previous_y,pacman_previous_orient
    pacman_x=random.randint(0,15)
    pacman_y=random.randint(0,15)
    pacman_orient=random.randint(0,3)
    pacman_previous_orient=pacman_orient
...
def repainting(pacman_direction=None,if_first=0):
    global pacman_flash_x,pacman_flash_y
    ...
    if if_pacmanmouse_open==1:
        if pacman_previous_orient==3:
            main_screen.blit(pacman_image_north,(pacman_flash_x*block_size+mapboarder_buffer,pacman_flash_y*block_size+mapboarder_buffer))
        if pacman_previous_orient==1:
            main_screen.blit(pacman_image_south,(pacman_flash_x*block_size+mapboarder_buffer,pacman_flash_y*block_size+mapboarder_buffer))
        if pacman_previous_orient==2:
            main_screen.blit(pacman_image_left,(pacman_flash_x*block_size+mapboarder_buffer,pacman_flash_y*block_size+mapboarder_buffer))
        if pacman_previous_orient==0:
            main_screen.blit(pacman_image_right,(pacman_flash_x*block_size+mapboarder_buffer,pacman_flash_y*block_size+mapboarder_buffer))
    else:
        main_screen.blit(pacman_image_none,(pacman_flash_x*block_size+mapboarder_buffer,pacman_flash_y*block_size+mapboarder_buffer))
    #painting ghost
    for i in range(ghost_num):
    ...
    pygame.draw.line(main_screen,(0,255,255),(mapboarder_buffer,mapboarder_buffer),(mapboarder_buffer,map_size*block_size+mapboarder_buffer),width=3)
    pygame.draw.line(main_screen,(0,255,255),(mapboarder_buffer,mapboarder_buffer),(map_size*block_size+mapboarder_buffer,mapboarder_buffer),width=3)
    pygame.draw.line(main_screen,(0,255,255),(map_size*block_size+mapboarder_buffer,mapboarder_buffer),(map_size*block_size+mapboarder_buffer,map_size*block_size+mapboarder_buffer),width=3)
    pygame.draw.line(main_screen,(0,255,255),(mapboarder_buffer,map_size*block_size+mapboarder_buffer),(map_size*block_size+mapboarder_buffer,map_size*block_size+mapboarder_buffer),width=3)
        ...
        if len(now_event)>0:
            ...
            for every_event in now_event:
                ...
                if every_event.type==27:
                    pacman_flash_x=pacman_previous_x+movingtick/10*(pacman_x-pacman_previous_x)
                    pacman_flash_y=pacman_previous_y+movingtick/10*(pacman_y-pacman_previous_y)
                    if movingtick>=10:
                        print("pacmanx:"+str(pacman_x))
                        print("pacmany:"+str(pacman_y))
                        movingtick-=10
                        pacman_previous_x=pacman_x
                        pacman_previous_y=pacman_y
                        pacman_previous_orient=pacman_orient
                        if pacmanmap_valid(pacman_x+direction_x[pacman_orient],pacman_y+direction_y[pacman_orient])==True:
                            pacman_x+=direction_x[pacman_orient]
                            pacman_y+=direction_y[pacman_orient]
                    if movingtick==5:
