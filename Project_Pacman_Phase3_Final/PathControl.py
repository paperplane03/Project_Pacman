...
def DFS_shortest_origin(nowx,nowy):
    ...
    if tempmap.get_ifeaten(nowx,nowy)==1:
        ...
        for i in range(4):
            ...
            if Pacman.pacmanmap_valid(nowx+tempvector[0],nowy+tempvector[1])==True and \
...
def BFS_shortest(nowx,nowy):
    ...
    while len(temp_BFSqueue)>0:
        ...
        if temp_front[2]>=bfs_visitmap[temp_x][temp_y]:
            continue
        if Pacman.pacmanmap_valid(temp_x,temp_y)==False:
            continue
        # print(temp_front)
        ...
        for i in range(4):
            tempvector=Directions.getmove(i)
            if Pacman.pacmanmap_valid(temp_x+tempvector[0],temp_y+tempvector[1])==True:
