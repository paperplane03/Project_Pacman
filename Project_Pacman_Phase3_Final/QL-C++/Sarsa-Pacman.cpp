...
//				if(best_strategy[i][j][0]==0)cout<<"下 ";
//				if(best_strategy[i][j][0]==1)cout<<"右 ";
//				if(best_strategy[i][j][0]==2)cout<<"上 ";
//				if(best_strategy[i][j][0]==3)cout<<"左 ";
...
void Sarsa_simu(int given_stra,int turn,bool if_test=false){
//	printf("simu:%d\n",turn);
	...
	for(int i=0;i<2;i++)ghostidx[i]=getnum(ghost[i][0],ghost[i][1]);
	
	// chosen_stra=epsilon_greedy_method(best_strategy[pacmanidx][ghostidx[0]][ghostidx[1]],turn);
	
	pacman_move(given_stra);
	ghost_move(0,lambda_chase(0.5,0));
	if(ghostnum==2)ghost_move(1,lambda_chase(0.5,1));
	
	new_beststra=epsilon_greedy_method(best_strategy[getnum(pacman[0],pacman[1])][getnum(ghost[0][0],ghost[0][1])][getnum(ghost[1][0],ghost[1][1])],0);
...
//		showway(ghost0way);
	...
	Q[pacmanidx][ghostidx[0]][ghostidx[1]][given_stra]=(1-studying_rate)*Q[pacmanidx][ghostidx[0]][ghostidx[1]][given_stra]+\
...
//	printf("Q:%f\n",Q[pacmanidx][ghostidx[0]][ghostidx[1]][chosen_stra]);
...
//	if_visited[nowx][nowy]=1;
	if(if_dead()==false)Sarsa_simu(new_beststra,turn+1,if_test);
	else{
		globalsum+=turn;
		if(if_test==true)cout<<"This turn:"<<turn<<endl;
	}
...
void Sarsa_algo(){
	...
	for(int i=1;i<=training_time;i++){
		...
		if(i%1000==0){
			u=round(win*((double)globalsum/1000));
			globalsum+=u;
			printf("%d:%f\n",i,(double)globalsum/1000);
			printf("%d:%f\n",i,(double)win/1000);
			globalsum=0;
			win=0;
		}if(i%10000==0)nowloading();
		agentsetup();
		pacmanidx=getnum(pacman[0],pacman[1]);
		if(ghostnum>0)ghostidx[0]=getnum(ghost[0][0],ghost[0][1]);
		else ghostidx[0]=0;
		if(ghostnum>1)ghostidx[1]=getnum(ghost[1][0],ghost[1][1]);
		else ghostidx[1]=0;
		int zerostra=epsilon_greedy_method(best_strategy[pacmanidx][ghostidx[0]][ghostidx[1]],0);
		Sarsa_simu(zerostra,1,false);
	}
...
int main(){
	...
	freopen("small-maze.txt","r",stdin);
	srand(time(NULL));
